...
def modify_seed_in_content(content, new_seed):
    ...
    if re.search(main_seed_pattern, content):
        ...
        seed_count = len(
            re.findall(main_seed_pattern.replace(r"\d+", str(new_seed)), content)
        )
        if seed_count > 1:
            print(
                f"  Updated {seed_count} MAIN_SEED occurrences (including config classes)"
            )
    else:
        ...
        content = re.sub(
            r"np\.random\.seed\(\d+\)", f"np.random.seed({new_seed})", content
        )
        content = re.sub(
            r"torch\.manual_seed\(\d+\)", f"torch.manual_seed({new_seed})", content
        )
        content = re.sub(
            r"torch\.cuda\.manual_seed\(\d+\)",
            f"torch.cuda.manual_seed({new_seed})",
            content,
        )
        content = re.sub(
            r"torch\.cuda\.manual_seed_all\(\d+\)",
            f"torch.cuda.manual_seed_all({new_seed})",
            content,
        )

        # Replace layer seed calculation (any_number + layer_idx pattern)
        content = re.sub(
            r"layer_seed = \d+ \+ layer_idx",
            f"layer_seed = {new_seed} + layer_idx",
            content,
        )

        # Replace the print statement (both literal numbers and variable references)
        content = re.sub(
            r"Random seeds set for reproducibility \(seed=\d+\)",
            f"Random seeds set for reproducibility (seed={new_seed})",
            content,
        )
        # Also handle cases where the print statement uses a variable like {MAIN_SEED}
        content = re.sub(
            r"Random seeds set for reproducibility \(seed=\{MAIN_SEED\}\)",
            f"Random seeds set for reproducibility (seed={new_seed})",
            content,
        )
...
def modify_seed_in_file(filepath, new_seed):
    ...
    if content == modified_content:
        ...
        if re.search(f"MAIN_SEED = {new_seed}", content):
            ...
            print(
                "  This might indicate the seed patterns were not found or script needs updating"
            )
    else:
...
def aggregate_results_group(results_list, method_name, metric_group, group_name):
            ...
            for col in metric_group:
                if col in group.columns:
                    ...
                    if len(values) > 0:
                        ...
                        result_row[f"{col}_Std"] = round(
                            values.std() if len(values) > 1 else 0.0, 4
                        )
    ...
    if not combined_df.empty and primary_metric in combined_df.columns:
        ...
        if "ML_" in combined_df["Method"].iloc[0] if len(combined_df) > 0 else False:
            ...
            rank_mapping = dict(
                zip(
                    zip(combined_df["Layer"], combined_df["Method"]),
                    combined_df[f"{group_name}_Rank"],
                )
            )
            df[f"{group_name}_Rank"] = df.apply(
                lambda row: rank_mapping.get((row["Layer"], row["Method"]), 0), axis=1
            )
        else:
            # MCD/KCD experiments: use Layer only (original behavior)
            rank_mapping = dict(
                zip(combined_df["Layer"], combined_df[f"{group_name}_Rank"])
            )
            df[f"{group_name}_Rank"] = df["Layer"].map(rank_mapping)
            df[f"{group_name}_Rank"] = df[f"{group_name}_Rank"].fillna(
                0
            )  # Fill NaN with 0 for non-COMBINED
    else:
...
def aggregate_results(results_list, method_name):
    ...
    group1_df = aggregate_results_group(
        results_list, method_name, group1_metrics, "Performance"
    )
    group2_df = aggregate_results_group(
        results_list, method_name, group2_metrics, "Classification"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run multiple experiments with different seeds"
    )
    parser.add_argument(
        "--script",
        required=True,
    ...
    parser.add_argument(
        "--runs", type=int, default=50, help="Number of runs (default: 50)"
    )
    parser.add_argument(
        "--seeds",
        nargs="+",
...
"""
    ...
    working_dir = (
        script_dir.parent
    )  # This is the parent directory (where experiments should run)
    ...
    if args.layers:
        try:
            parsed_layers = [
                int(part.strip()) for part in args.layers.split(",") if part.strip()
            ]
        except ValueError:
    ...
    if args.resume:
        ...
        existing_dir = find_newest_matching_directory(
            output_dir, args.script, args.model
        )
        ...
        if existing_dir:
            ...
            if metadata:
                existing_seeds = metadata.get("seeds", [])[
                    :existing_runs
                ...
                if layer_subset is None and stored_layers:
                    ...
                    if layer_subset:
                        layer_subset_str = ",".join(
                            str(layer) for layer in layer_subset
                        )
                    print(f"Resuming with stored layer subset: {layer_subset}")
                elif (
                    layer_subset is not None
                    and stored_layers
                    and sorted(stored_layers) != sorted(layer_subset)
                ):
                    print(
                        "Warning: Provided --layers differs from stored layer subset in the existing experiment."
                    )

            if existing_runs < args.runs:
                print(
                    f"Resuming from run {existing_runs + 1} (need {args.runs - existing_runs} more runs)"
                )
                run_dir = existing_dir
                resume_from_run = existing_runs + 1
            else:
                print(
                    f"Existing directory already has {existing_runs} runs (>= requested {args.runs})"
                )
                print(
                    "No additional runs needed. Use a different --runs value or create a new experiment."
                )
                return
        else:
    ...
    if args.seeds:
        if args.resume and existing_runs > 0:
            ...
            if len(remaining_seeds) < (args.runs - existing_runs):
                print(
                    f"Warning: Not enough custom seeds for remaining runs. Need {args.runs - existing_runs}, got {len(remaining_seeds)}"
                )
            ...
            if len(seeds) < args.runs:
                print(
                    f"Warning: Only {len(seeds)} seeds provided, but {args.runs} runs requested"
                )
                args.runs = len(seeds)
    else:
        if args.resume and existing_runs > 0:
            ...
            last_seed = (
                existing_seeds[-1] if existing_seeds else 41
            )  # Default to 41 so next is 42
            seeds = list(
                range(last_seed + 1, last_seed + 1 + (args.runs - existing_runs))
            )
        else:
            seeds = list(
                range(42, 42 + args.runs)
            )  # Use consecutive seeds starting from 42
    ...
    if args.resume and existing_runs > 0:
        ...
        existing_run_count, existing_run_files = count_successful_runs(
            run_dir, expected_csv
        )
    ...
    for i, seed in enumerate(seeds, resume_from_run):
        ...
        if success:
            ...
            if not csv_source.exists():
                ...
                alt_paths = [
                    working_dir
                    / expected_csv.split("/")[-1],  # Just filename in working_dir
                    script_dir
                    / expected_csv.split("/")[-1],  # Just filename in script_dir
        ...
        print(
            f"Completed {current_run_in_sequence}/{total_new_runs} new runs ({successful_runs} total successful)"
        )
    ...
    if group1_df is not None and group2_df is not None:
        ...
        combined_perf = (
            group1_df[group1_df["Dataset"] == "COMBINED"]
            .sort_values("Performance_Rank")
            .head(5)
        )
        combined_class = (
            group2_df[group2_df["Dataset"] == "COMBINED"]
            .sort_values("Classification_Rank")
            .head(5)
        )
        ...
        if not combined_perf.empty:
            ...
            for _, row in combined_perf.iterrows():
                print(
                    f"  Layer {int(row['Layer'])}: Accuracy={row['Accuracy_Mean']:.4f}±{row['Accuracy_Std']:.4f}"
                )
        ...
        if not combined_class.empty:
            ...
            for _, row in combined_class.iterrows():
                print(
                    f"  Layer {int(row['Layer'])}: F1={row['F1_Mean']:.4f}±{row['F1_Std']:.4f}"
                )
    else:
