...
class HiddenStateExtractor:
    ...
    def _prepare_multimodal_inputs(self, text: str, image_path: str):
        ...
        image_tokens = (
            ...
            + (
                IMG_CONTEXT_TOKEN
                * (getattr(self.model, "num_image_token") * num_patches)
            )
            + IMG_END_TOKEN
        )
        query = text or ""
        ...
        image_flags = torch.ones(
            (num_patches, 1), device=self.model.device, dtype=torch.long
        )
    ...
    def extract_hidden_states(
                ...
                try:
                    if (batch_start + idx) % memory_cleanup_freq == 0:
                        ...
                        if mem["gpu_allocated"] > 20:
                            print(
                                f"High GPU memory usage: {mem['gpu_allocated']:.1f}GB allocated, {mem['gpu_cached']:.1f}GB cached"
                            )
                    ...
                    with torch.inference_mode():
                        if img_path is not None:
                            input_ids, attention_mask, pixel_values, image_flags = (
                                self._prepare_multimodal_inputs(text, img_path)
                            )
                            out = self.model(
                                input_ids=input_ids,
        ...
        print(
            f"Feature extraction completed. GPU: {final_mem['gpu_allocated']:.1f}GB alloc, {final_mem['gpu_cached']:.1f}GB cached; CPU: {final_mem['cpu_used']:.1f}GB used"
        )
