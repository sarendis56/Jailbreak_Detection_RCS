        ...
        if len(all_benign_features) >= k_benign:
            ...
            for cluster_id in range(k_benign):
                ...
                if len(cluster_features) > 0:
                    ...
                    print(f"    Benign cluster {cluster_id}: {len(cluster_features)} samples")
        else:
            print(f"  Warning: Not enough benign samples ({len(all_benign_features)}) for k={k_benign} clusters")
        ...
        if len(all_malicious_features) >= k_malicious:
            ...
            for cluster_id in range(k_malicious):
                ...
                if len(cluster_features) > 0:
                    ...
                    print(f"    Malicious cluster {cluster_id}: {len(cluster_features)} samples")
        else:
            print(f"  Warning: Not enough malicious samples ({len(all_malicious_features)}) for k={k_malicious} clusters")
            # Fallback: use all samples as one cluster
...
def main():
    ...
    print("="*80)
    print(f"BALANCED OOD-BASED JAILBREAK DETECTION USING MCD WITH K-MEANS CLUSTERING ({model_type.upper()})")
    print("="*80)
    print(f"Model: {model_path}")
    print(f"Model type: {model_type}")
    print("Approach: Aggregate all benign/malicious samples across datasets")
    print("          Use k-means clustering to model k Gaussians (k = number of datasets)")
    ...
    for layer_idx in layers:
        ...
        # Prepare k-means based data structures (aggregate all data, then cluster)
        # Combine both in-distribution and OOD datasets for k-means clustering
        ...
        print(f"  Using k-means clustering approach instead of dataset-based grouping...")
        in_dist_data, ood_train_data = prepare_kmeans_data_structure(
            all_training_datasets,
            training_layer_hidden_states,
            training_layer_labels,
            random_seed=MAIN_SEED + layer_idx  # Layer-specific seed for reproducibility
        )
    ...
    # Save results to CSV
    output_path = f"results/balanced_mcd_kmeans_{model_type}_results.csv"
