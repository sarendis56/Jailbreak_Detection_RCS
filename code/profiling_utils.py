...
class PerformanceProfiler:
    ...
    def _get_memory_usage(self):
        ...
        if torch.cuda.is_available():
            ...
            memory_info["gpu_max_allocated_mb"] = (
    ...
    def profile(self, operation_name, num_samples=None):
        ...
        try:
            ...
            gpu_memory_delta = (
                mem_after["gpu_allocated_mb"] - mem_before["gpu_allocated_mb"]
            )
            ...
            measurement = {
                ...
                "time_per_sample_seconds": elapsed_time / num_samples
                if num_samples
                else None,
...
def save_profiling_results(
    ...
    with open(output_path, "a", newline="") as csvfile:
        ...
        feature_extraction = feature_extraction_summary or profiler.get_summary(
            "feature_extraction"
        )
        ...
        if projection:
            ...
            if proj_measurements:
                num_samples = sum(
                    m.get("num_samples", 0) or 0 for m in proj_measurements
                )
            if num_samples == 0 and projection.get("mean_time_seconds", 0) > 0:
                # Fallback: estimate from time
                num_samples = int(
                    projection["total_time_seconds"] / projection["mean_time_seconds"]
                )
            throughput = (
                num_samples / projection["total_time_seconds"]
                if projection["total_time_seconds"] > 0 and num_samples > 0
                else 0
            )
            time_per_sample_ms = (
                projection["mean_time_seconds"] * 1000
                if projection["mean_time_seconds"]
                else 0
            )
            peak_gpu_gb = (
                projection.get(
                    "peak_gpu_memory_mb", projection.get("mean_gpu_memory_mb", 0)
                )
                / 1024
            )  # Convert MB to GB
            peak_cpu_gb = (
                projection.get("mean_cpu_memory_mb", 0) / 1024
            )  # Convert MB to GB
            ...
            row = {
                ...
                "Throughput_Samples_per_sec": f"{throughput:.6f}"
                if throughput > 0
                else "",
                "Time_per_Sample_ms": f"{time_per_sample_ms:.6f}"
                if time_per_sample_ms > 0
                else "",
            }
            writer.writerow(row)
        ...
        if detection:
            ...
            if det_measurements:
                num_samples = sum(
                    m.get("num_samples", 0) or 0 for m in det_measurements
                )
            if num_samples == 0 and detection.get("mean_time_seconds", 0) > 0:
                # Fallback: estimate from time
                num_samples = int(
                    detection["total_time_seconds"] / detection["mean_time_seconds"]
                )
            throughput = (
                num_samples / detection["total_time_seconds"]
                if detection["total_time_seconds"] > 0 and num_samples > 0
                else 0
            )
            time_per_sample_ms = (
                detection["mean_time_seconds"] * 1000
                if detection["mean_time_seconds"]
                else 0
            )
            peak_gpu_gb = (
                detection.get(
                    "peak_gpu_memory_mb", detection.get("mean_gpu_memory_mb", 0)
                )
                / 1024
            )  # Convert MB to GB
            peak_cpu_gb = (
                detection.get("mean_cpu_memory_mb", 0) / 1024
            )  # Convert MB to GB

            component_name = (
                "knn_distance" if method_name == "KCD" else "mahal_distance"
            )
            row = {
                "Method": method_name,
        ...
        if baseline_summary:
            ...
            throughput = (
                num_samples / baseline_summary["total_time_seconds"]
                if baseline_summary["total_time_seconds"] > 0 and num_samples > 0
                else 0
            )
            time_per_sample_ms = (
                baseline_summary["mean_time_seconds"] * 1000
                if baseline_summary["mean_time_seconds"]
                else 0
            )
            peak_gpu_gb = (
                baseline_summary.get("peak_gpu_memory_mb", 0) / 1024
            )  # Convert MB to GB
            peak_cpu_gb = (
                baseline_summary.get("mean_cpu_memory_mb", 0) / 1024
            )  # Convert MB to GB
        ...
        if feature_extraction:
            ...
            throughput = (
                num_samples / feature_extraction["total_time_seconds"]
                if feature_extraction["total_time_seconds"] > 0 and num_samples > 0
                else 0
            )
            time_per_sample_ms = (
                feature_extraction["mean_time_seconds"] * 1000
                if feature_extraction["mean_time_seconds"]
                else 0
            )
            peak_gpu_gb = (
                feature_extraction.get(
                    "peak_gpu_memory_mb",
                    feature_extraction.get("mean_gpu_memory_mb", 0),
                )
                / 1024
            )  # Convert MB to GB
            peak_cpu_gb = (
                feature_extraction.get("mean_cpu_memory_mb", 0) / 1024
            )  # Convert MB to GB
