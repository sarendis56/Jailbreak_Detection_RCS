...
class LayerDiscriminativeAnalyzer:
    ...
    def __init__(self, model_path: str, model_type: str):
        ...
        if model_type == "qwen":
            from feature_extractor_qwen import (
                HiddenStateExtractor as QwenHiddenStateExtractor,
            )

            self.extractor = QwenHiddenStateExtractor(model_path)
        elif model_type == "llava":
            from feature_extractor import (
                HiddenStateExtractor as LLaVAHiddenStateExtractor,
            )

            self.extractor = LLaVAHiddenStateExtractor(model_path)
        elif model_type == "internvl":
            try:
                from code.feature_extractor_internvl import (
                    HiddenStateExtractor as InternVLHiddenStateExtractor,
                )
            except ImportError:
                from feature_extractor_internvl import (
                    HiddenStateExtractor as InternVLHiddenStateExtractor,
                )
            self.extractor = InternVLHiddenStateExtractor(model_path)
        else:
            raise ValueError(
                f"Unknown model_type: {model_type}. Use one of: 'qwen', 'llava', 'internvl'."
            )
        self.results = {}
    ...
    def load_sgxstest_dataset(self) -> List[Dict]:
        ...
        try:
            ...
            print(
                f"Dataset composition: {benign_count} benign, {malicious_count} malicious"
            )
    ...
    def load_noisy_distribution_dataset(self, n_samples: int = 100) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "Failed to load benign samples for noisy distribution test"
                )
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "Failed to load malicious samples for noisy distribution test"
                )

        benign_count = sum(1 for s in samples if s["toxicity"] == 0)
        malicious_count = sum(1 for s in samples if s["toxicity"] == 1)
        print(
            f"  Total: {len(samples)} samples ({benign_count} benign, {malicious_count} malicious)"
        )
    ...
    def load_latent_neighbor_dataset(self, n_pairs: int = 100) -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                if len(malicious_pool) > n_pairs:
                    ...
                    malicious_samples = malicious_pool[:n_pairs]
                print(
                    f"  Loaded {len(malicious_samples)} malicious prompts from DAN Prompts"
                )
            except Exception as e2:
        ...
        try:
            ...
            if len(benign_pool) < n_pairs:
                ...
                try:
                    ...
                    pass
            print(
                f"  Loaded {len(benign_pool)} benign prompts for nearest neighbor search"
            )
        except Exception as e:
            print(f"  Warning: Could not load Alpaca: {e}")
            try:
        ...
        try:
            ...
            malicious_embeddings = embedder.encode(
                malicious_texts, show_progress_bar=False
            )
        ...
        avg_similarity = np.mean(
            [s.get("pair_similarity", 0) for s in samples if "pair_similarity" in s]
        )
    ...
    def load_in_the_wild_dataset(self) -> List[Dict]:
        ...
        try:
            ...
            llm_attack = load_JailBreakV_custom(
                attack_types=["llm_transfer_attack"], max_samples=275
            )
            query_related = load_JailBreakV_custom(
    ...
    def analyze_layer(
        self, layer_idx: int, features: np.ndarray, labels: np.ndarray
    ) -> Dict[str, float]:
        """
        ...
        results["wasserstein"] = self.compute_wasserstein_distance(
        ...
        results["entropy_reduction"] = self.compute_conditional_entropy_reduction(
            features, labels
        )
    ...
    def run_comprehensive_analysis(
        ...
        for layer_idx in range(layer_start, layer_end + 1):
            ...
            layer_results[layer_idx] = self.analyze_layer(
                layer_idx, features, np.array(labels)
            )
    ...
    def compute_composite_scores(self, results: Dict) -> Dict[int, Dict[str, float]]:
        ...
        for metric in weights.keys():
            ...
            if all(v == 0 for v in metric_values):
                ...
                if iqr == 0:
                    ...
                    normalized_values = [
                        1 / (1 + np.exp(-2 * rs)) for rs in robust_scaled
                    ]
        ...
        for layer in all_layers:
            ...
            geometric_score = (
                ...
                * normalized_results[layer]["distance_ratio"]
            ) / (
                weights["svm_margin"]
                + weights["silhouette"]
                + weights["distance_ratio"]
            ...
            information_score = (
                ...
                + weights["entropy_reduction"]
                * normalized_results[layer]["entropy_reduction"]
    ...
    def print_summary_report(self, ranking: List[Tuple]):
        ...
        print("\n" + "=" * 100)
        print(
            "PRINCIPLED LAYER SELECTION FOR JAILBREAK DETECTION - COMPREHENSIVE REPORT"
        )
        print("=" * 100)
        ...
        print(f"-" * 100)
        print(
            f"{'Rank':<4} {'Layer':<5} {'Overall':<8} {'Distrib':<8} {'Geometric':<9} {'Info':<8} {'Recommendation'}"
        )
        print(f"-" * 100)

        for rank, (layer_idx, overall_score, detailed_scores) in enumerate(
            ranking[:10], 1
        ):
            ...
            print(
                f"{rank:<4} {layer_idx:<5} {overall_score:.3f}    {dist_score:.3f}    {geom_score:.3f}     {info_score:.3f}    {recommendation}"
            )
        ...
        print(
            f"  Distributional Divergence Score: {best_details['distributional_score']:.4f}"
        )
        ...
        print(f"")
        print(
            f"4. ENSEMBLE APPROACH: Consider combining top 3-5 layers for robust detection"
        )
        print(
            f"5. VALIDATION: Test selected layer(s) on your specific jailbreak detection task"
        )
        print(f"")
        print(
            f"This analysis provides a principled, data-driven foundation for layer selection"
        )
    ...
    def print_detailed_rankings(self, composite_scores: Dict):
        ...
        for score_key, category_name in categories:
            ...
            category_ranking = [
                (layer, composite_scores[layer][score_key]) for layer in layers
            ]
        ...
        for metric_key, metric_name in metrics:
            ...
            metric_ranking = [
                (layer, composite_scores[layer][metric_key]) for layer in layers
            ]
        ...
        consistent_layers = [
            (layer, appearances) for layer, appearances in top_5_appearances.items()
        ]
        ...
        for layer, appearances in consistent_layers[:10]:
            if (
                len(appearances) >= 2
            ):  # Only show layers that appear in top 5 of at least 2 metrics
                metrics_str = ", ".join(
                    [f"{metric}({rank})" for metric, rank in appearances[:4]]
                )
        ...
        print(
            "• Layers with high 'Appearances' are consistently good across multiple metrics"
        )
...
def print_experiment_summary(exp_num: int, exp_name: str, ranking: List[Tuple]):
    ...
    print(
        f"  Experiment {exp_num} ({exp_name}): Best layer = {best_layer} (score: {best_score:.4f})"
    )
...
def analyze_correlations(all_experiment_results):
    ...
    for i, exp1 in enumerate(exp_names):
        for exp2 in exp_names[i + 1 :]:
            scores1 = [
                experiment_scores[exp1][layer]
                for layer in sorted(experiment_scores[exp1].keys())
            ]
            scores2 = [
                experiment_scores[exp2][layer]
                for layer in sorted(experiment_scores[exp2].keys())
            ]
...
def create_correlation_visualization(all_experiment_results, analyzer=None):
    ...
    try:
        ...
        fig.suptitle(
            "Robustness Experiments: Layer Selection Comparison",
            fontsize=16,
            fontweight="bold",
        )
        ...
        if len(experiment_scores) >= 2:
            ...
            ax2.set_xticklabels(
                [name.replace("_", " ").title() for name in exp_names],
                rotation=45,
                ha="right",
            )
        ...
        for exp_name, layer_scores in experiment_scores.items():
            ...
            ax3.bar(
                ...
                - 0.2
                * (
                    list(experiment_scores.keys()).index(exp_name)
                    - len(experiment_scores) / 2
                    + 0.5
                ),
                top_scores,
                width=0.2,
...
def save_experiment_results(exp_data: Dict, model_name: str):
    ...
    if exp_num == 1:
        ...
        output_path = (
            f"results/principled_layer_selection_results_{exp_name}_{model_name}.csv"
        )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python principled_layer_selection.py <model_type> [model_path] [experiments]"
        )
    ...
    if model_type not in MODEL_PATHS:
        print(
            f"Error: Unknown model type '{model_type}'. Use one of: {list(MODEL_PATHS.keys())}"
        )
        sys.exit(1)
    ...
    if not os.path.exists(model_path):
        ...
        print(
            f"Please download the model using download_models.py or provide the model path manually"
        )
        sys.exit(1)
    ...
    print(
        f"Layer range: {layer_start}-{layer_end} ({layer_end - layer_start + 1} layers)"
    )
    ...
    for exp_num in exp_nums:
        ...
        if exp_result is not None:
            print_experiment_summary(
                exp_num, exp_result["exp_name"], exp_result["ranking"]
            )
...
def create_visualization(ranking: List[Tuple], composite_scores: Dict, analyzer=None):
    ...
    try:
        ...
        fig.suptitle(
            "Comprehensive Principled Layer Selection Analysis for Jailbreak Detection",
            fontsize=18,
            fontweight="bold",
            y=0.98,
        )
        ...
        dist_scores = [
            composite_scores[layer]["distributional_score"] for layer in top_10_layers
        ]
        geom_scores = [
            composite_scores[layer]["geometric_score"] for layer in top_10_layers
        ]
        info_scores = [
            composite_scores[layer]["information_score"] for layer in top_10_layers
        ]
        ...
        create_individual_metric_rankings(
            metric_axes, composite_scores, layers, metrics[:4]
        )
        ...
        create_second_figure_with_remaining_metrics(
            composite_scores, layers, metrics[4:], analyzer
        )
...
def create_category_rankings_plot(ax, composite_scores, layers):
    ...
    dist_scores = [
        (layer, composite_scores[layer]["distributional_score"]) for layer in layers
    ]
    geom_scores = [
        (layer, composite_scores[layer]["geometric_score"]) for layer in layers
    ]
    info_scores = [
        (layer, composite_scores[layer]["information_score"]) for layer in layers
    ]
    ...
    ax.bar(
        x_pos - width,
        [score for _, score in dist_scores[:top_n]],
        width,
        label="Distributional",
        alpha=0.8,
        color="lightcoral",
    ...
    ax.bar(
        x_pos,
        [score for _, score in geom_scores[:top_n]],
        width,
        label="Geometric",
        alpha=0.8,
        color="sandybrown",
    ...
    ax.bar(
        x_pos + width,
        [score for _, score in info_scores[:top_n]],
        width,
        label="Information",
        alpha=0.8,
        color="lightgreen",
...
def create_individual_metric_rankings(axes, composite_scores, layers, metrics):
    ...
    for i, (ax, metric) in enumerate(zip(axes, metrics)):
        ...
        bars = ax.bar(
            range(len(top_10)), top_scores, alpha=0.7, color=colors[i % len(colors)]
        )
...
def create_second_figure_with_remaining_metrics(
    composite_scores, layers, remaining_metrics, analyzer=None
):
    """Create second figure for remaining individual metrics."""
    fig2, axes2 = plt.subplots(2, 2, figsize=(12, 10))
    fig2.suptitle(
        "Individual Metric Rankings (Continued)", fontsize=14, fontweight="bold"
    )
    ...
    for i, (ax, metric) in enumerate(zip(axes2_flat, remaining_metrics)):
        ...
        bars = ax.bar(
            range(len(top_10)), top_scores, alpha=0.7, color=colors2[i % len(colors2)]
        )
