...
class EmbeddingVisualizer:
    ...
    def __init__(
        self,
        llava_model_path="model/llava-v1.6-vicuna-7b/",
        flava_model_path="model/flava",
    ):
        self.llava_model_path = llava_model_path
        self.flava_model_path = flava_model_path
    ...
    def _load_extractors(self):
        ...
        if self.flava_extractor is None:
            ...
            self.flava_extractor = FlavaFeatureExtractor(self.flava_model_path)
    ...
    def load_datasets_for_visualization(self):
        ...
        try:
            ...
            if mmvet_samples:
                mmvet_subset = mmvet_samples[:300]
                datasets["MM-Vet"] = {
                    "samples": mmvet_subset,
                    "label": 0,
                    "color": "blue",
                }
        ...
        try:
            jbv_figstep_samples = load_JailBreakV_figstep(max_samples=300)
            if jbv_figstep_samples:
                datasets["FigStep"] = {
                    "samples": jbv_figstep_samples,
                    "label": 1,
                    "color": "red",
                }
        ...
        try:
            vqav2_samples = load_vqav2(max_samples=300)
            if vqav2_samples:
                datasets["VQAv2"] = {
                    "samples": vqav2_samples,
                    "label": 0,
                    "color": "lightblue",
                }
        ...
        try:
            alpaca_samples = load_alpaca(max_samples=300)
            if alpaca_samples:
                datasets["Alpaca"] = {
                    "samples": alpaca_samples,
                    "label": 0,
                    "color": "green",
                }
        ...
        try:
            openassistant_samples = load_openassistant(max_samples=300)
            if openassistant_samples:
                datasets["OpenAssistant"] = {
                    "samples": openassistant_samples,
                    "label": 0,
                    "color": "lightgreen",
                }
        ...
        try:
            jbv_llm_samples = load_JailBreakV_custom(
                attack_types=["llm_transfer_attack", "query_related"], max_samples=300
            )
            if jbv_llm_samples:
                datasets["JailbreakV-LLM"] = {
                    "samples": jbv_llm_samples,
                    "label": 1,
                    "color": "darkred",
                }
        ...
        try:
            ...
            if xstest_samples:
                xstest_safe = [s for s in xstest_samples if s.get("toxicity", 0) == 0][
                    :150
                ]
                if xstest_safe:
                    datasets["XSTest-Safe"] = {
                        "samples": xstest_safe,
                        "label": 0,
                        "color": "cyan",
                    }
        ...
        try:
            if "xstest_samples" in locals():
                xstest_unsafe = [
                    s for s in xstest_samples if s.get("toxicity", 0) == 1
                ][:150]
                if xstest_unsafe:
                    datasets["XSTest-Unsafe"] = {
                        "samples": xstest_unsafe,
                        "label": 1,
                        "color": "magenta",
                    }
        ...
        try:
            mm_safety_samples = load_mm_safety_bench_all(max_samples=300)
            if mm_safety_samples:
                datasets["MM-SafetyBench"] = {
                    "samples": mm_safety_samples,
                    "label": 1,
                    "color": "brown",
                }
        ...
        for dataset_name, dataset_info in datasets.items():
            ...
            try:
                if model_type == "llava":
                    ...
                    hidden_states, labels, _ = (
                        self.llava_extractor.extract_hidden_states(
                            samples,
                            f"{dataset_name}_viz",
                            layer_start=16,
                            layer_end=16,
                    ...
                    embeddings = np.array(
                        hidden_states[16]
                    )  # Get layer 16 embeddings and convert to numpy array

                elif model_type == "flava":
                    # Extract Flava embeddings
                    embeddings, labels = self.flava_extractor.extract_features(
    ...
    def create_pca_visualization(
        ...
        for combo_name, dataset_names in dataset_combinations.items():
            ...
            plt.xlabel(
                f"PC1 ({pca.explained_variance_ratio_[0]:.1%} variance)", fontsize=42
            )
            plt.ylabel(
                f"PC2 ({pca.explained_variance_ratio_[1]:.1%} variance)", fontsize=42
            )
            plt.title(
                f"PCA Visualization: {combo_name}\n{model_type.upper()} Embeddings (Layer 16)"
                if model_type == "llava"
                else f"PCA Visualization: {combo_name}\n{model_type.upper()} Embeddings",
                fontsize=45,
            )
            plt.legend(
    ...
    def create_combined_pca_visualization(
        self,
        ...
        # Create a unified color mapping for all datasets
        all_datasets = set()
        for dataset_names in dataset_combinations.values():
            all_datasets.update(dataset_names)
        ...
        unified_colors = {
            ...
            "AdvBench": "orange",
            "Alpaca": "green",
            "OpenAssistant": "lightgreen",
            "JailbreakV-LLM": "darkred",
            "DAN": "purple",
            "XSTest-Safe": "cyan",
            "XSTest-Unsafe": "magenta",
            "MM-SafetyBench": "brown",
        }
        ...
        for row_idx, (model_name, embeddings_dict) in enumerate(model_data):
            for col_idx, combo_name in enumerate(combo_names):
                ...
                for dataset_name in dataset_names:
                    if dataset_name in embeddings_dict:
                        ...
                        all_dataset_names.extend(
                            [dataset_name] * len(data["embeddings"])
                        )
                ...
                if len(all_embeddings) == 0:
                    ...
                    ax.set_title(
                        f"{model_name}: {combo_name}", fontsize=30, fontweight="bold"
                    )
                    continue
                ...
                # Plot each dataset with unified colors
                unique_datasets = list(set(all_dataset_names))
                for dataset_name in unique_datasets:
                    mask = np.array(all_dataset_names) == dataset_name
                    # Use unified color scheme
                    color = unified_colors.get(dataset_name, "gray")
                    ax.scatter(
                        X_pca[mask, 0],
                        X_pca[mask, 1],
                        c=color,
        ...
        # Create a unified legend with all datasets that appear in any subplot
        all_unique_datasets = set()
        for dataset_names in dataset_combinations.values():
            for dataset_name in dataset_names:
                if (
                    dataset_name in llava_embeddings_dict
                    or dataset_name in flava_embeddings_dict
                ):
                    all_unique_datasets.add(dataset_name)

        # Create legend handles manually using the unified color scheme
        legend_handles = []
        legend_labels = []
        for dataset_name in sorted(all_unique_datasets):
            color = unified_colors.get(dataset_name, "gray")
            handle = Line2D(
                [0],
                [0],
        ...
        # Add the unified legend to the figure
        fig.legend(
            legend_handles,
            legend_labels,
            loc="center right",
        ...
        # Adjust layout with more spacing and room for external legend
        plt.tight_layout()
        plt.subplots_adjust(hspace=0.3, wspace=0.4, right=0.82)
    ...
    def run_visualization_analysis(self):
        ...
        combinations = {
            "MM-Vet & FigStep": ["MM-Vet", "FigStep"],
            "Basic Datasets": [
                "MM-Vet",
                "FigStep",
                "Alpaca",
                "AdvBench",
            ],  # One text-only and one multimodal for each category
            "JailDAM": [
                "MM-Vet",
                "MM-SafetyBench",
                "FigStep",
                "JailbreakV-LLM",
            ],  # JailDAM experiment setup
            "All Datasets": [
                "MM-Vet",
                "FigStep",
                "VQAv2",
        ...
        if len(llava_embeddings_dict) > 0 and len(flava_embeddings_dict) > 0:
            ...
            print(f"{'=' * 60}")
            self.create_combined_pca_visualization(
                llava_embeddings_dict, flava_embeddings_dict, combinations
            )
        ...
        print("1. MM-Vet & FigStep: Basic separability between benign and malicious")
        print("2. Basic Datasets: How separability changes with more datasets")
        print(
            "3. JailDAM: Previous work setup (MM-Vet vs MM-SafetyBench, FigStep, JailbreakV-28k)"
        )
        print("4. All Datasets: Full complexity with all available datasets")
        print("\nCombined visualization (saved in both PDF and PNG formats):")
        print(
            "- combined_pca_visualization.pdf/.png: 2x4 subplot showing all models and cases in one figure"
        )
