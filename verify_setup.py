...
def get_models_to_download():
    try:
        ...
        return getattr(download_models, "MODELS_TO_DOWNLOAD", {}), getattr(
            download_models, "MODEL_SIZES", {}
        )
...
def check_xstest_format():
    ...
    if not PANDAS_AVAILABLE:
        ...
        if os.path.exists(json_path):
            ...
            print(
                f"    [INFO] Install pandas for full format verification: pip install pandas"
            )
            return True
        else:
    ...
    if parquet_exists:
        ...
        try:
            ...
            if all(col in df.columns for col in required_cols):
                ...
                print(
                    f"    [SUCCESS] Distribution: {safe_count} safe, {contrast_count} unsafe samples"
                )
                return True
            else:
                print(
                    f"    [ERROR] Missing required columns. Found: {list(df.columns)}"
                )
                return False
        except Exception as e:
...
def verify_datasets():
            ...
            if check_directory_exists(filepath):
                ...
                try:
                    ...
                    if parquet_count > 0:
                        ...
                        print(
                            f"    [WARNING] Directory exists but no parquet files found"
                        )
                except Exception as e:
    ...
    vizwiz_files = ["data/VQAv2/vqav2_samples.json", "data/VQAv2/images"]
    ...
    if original_vqav2_count == len(original_vqav2_files):
        print(
            f"      [SUCCESS] Original VQAv2 format complete ({original_vqav2_count}/{len(original_vqav2_files)} components)"
        )
        datasets_status["VQAv2"] = True
        auto_count += 1
    elif vizwiz_count == len(vizwiz_files):
        print(
            f"      [SUCCESS] VizWiz-VQA format complete ({vizwiz_count}/{len(vizwiz_files)} components)"
        )
        ...
        print(
            f"        Original VQAv2: {original_vqav2_count}/{len(original_vqav2_files)} components"
        )
    ...
    manual_dirs = [
        ...
        (
            "data/JailBreakV-28k/llm_transfer_attack",
            "JailBreakV-28k LLM transfer attack images",
        ),
    ...
    print(
        f"  Automatic datasets: {auto_count}/{len(auto_datasets) + 2} [SUCCESS]"
    )  # +2 for VQAv2 and XSTest
    ...
    total_required = (
        len(auto_datasets) + 2 + len(manual_datasets) + len(manual_dirs)
    )  # +2 for VQAv2 and XSTest
...
def verify_models():
    ...
    model_mapping = {
        "llava_vicuna_7b": (
            ...
            "LLaVA-v1.6-Vicuna-7B multimodal model",
        ),
        ...
        "clip_large": (
            ...
            "CLIP ViT Large model (LLaVA vision tower)",
        ),
    ...
    if disabled_models:
        print(
            f"  Disabled models: {len(disabled_models)} (edit download_models.py to enable)"
        )
...
def test_basic_loading():
    ...
    try:
        ...
        from load_datasets import (
            ...
            load_openassistant,
            load_FigTxt,
            load_JailBreakV_figstep,
            load_adversarial_img,
        ...
        try:
            ...
            if alpaca_samples:
                print(
                    f"    [SUCCESS] Alpaca: Loaded {len(alpaca_samples)} test samples"
                )
            else:
        ...
        try:
            ...
            if advbench_samples:
                print(
                    f"    [SUCCESS] AdvBench: Loaded {len(advbench_samples)} test samples"
                )
            else:
        ...
        try:
            ...
            if dan_samples:
                print(
                    f"    [SUCCESS] DAN Prompts: Loaded {len(dan_samples)} test samples"
                )
            else:
        ...
        try:
            ...
            if oa_samples:
                print(
                    f"    [SUCCESS] OpenAssistant: Loaded {len(oa_samples)} test samples"
                )
            else:
        ...
        try:
            ...
            if xstest_samples:
                safe_count = len(
                    [s for s in xstest_samples if s.get("toxicity", 0) == 0]
                )
                unsafe_count = len(
                    [s for s in xstest_samples if s.get("toxicity", 0) == 1]
                )
                print(
                    f"    [SUCCESS] XSTest: Loaded {len(xstest_samples)} samples ({safe_count} safe, {unsafe_count} unsafe)"
                )
            else:
        ...
        try:
            ...
            if figtxt_samples:
                safe_count = len(
                    [s for s in figtxt_samples if s.get("toxicity", 0) == 0]
                )
                unsafe_count = len(
                    [s for s in figtxt_samples if s.get("toxicity", 0) == 1]
                )
                print(
                    f"    [SUCCESS] FigTxt: Loaded {len(figtxt_samples)} samples ({safe_count} safe, {unsafe_count} unsafe)"
                )
            else:
        ...
        try:
            ...
            if jbv_samples:
                print(
                    f"    [SUCCESS] JailbreakV-28K (figstep): Loaded {len(jbv_samples)} test samples"
                )
            else:
        ...
        try:
            ...
            if vae_samples:
                print(
                    f"    [SUCCESS] VAE (adversarial images): Loaded {len(vae_samples)} samples"
                )
            else:
        ...
        try:
            ...
            if mm_safety_samples:
                print(
                    f"    [SUCCESS] MM-SafetyBench: Loaded {len(mm_safety_samples)} test samples"
                )
            else:
            ...
            if os.path.exists(config_path):
                try:
                    with open(config_path, "r") as f:
                        ...
                        print(
                            f"  [SUCCESS] LLaVA config accessible (type: {model_type})"
                        )
    ...
    for model_key, model_path in [
        ...
        if models_to_download.get(model_key, False):
                ...
                if os.path.exists(config_path):
                    try:
                        with open(config_path, "r") as f:
                            ...
                            model_type = config.get("model_type", "unknown")
                            print(
                                f"  [SUCCESS] {model_key.upper()} config accessible (type: {model_type})"
                            )
                    except Exception as e:
            ...
            print(
                f"  [SKIP] {model_key.upper()} testing skipped (disabled in configuration)"
            )
        ...
        if os.path.exists(internvl_path):
            ...
            if os.path.exists(config_path):
                try:
                    ...
                    print(
                        f"  [SUCCESS] InternVL3-8B config accessible (model_type: {getattr(config, 'model_type', 'unknown')})"
                    )
...
def main():
    ...
    dataset_percent = (
        (dataset_available / dataset_total) * 100 if dataset_total > 0 else 0
    )
    model_percent = (model_available / model_total) * 100 if model_total > 0 else 0

    print(
        f"[DATASETS] {dataset_available}/{dataset_total} ({dataset_percent:.0f}%) available"
    )
